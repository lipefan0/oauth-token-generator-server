...
export async function createAccount(req, res) {
  ...
  try {
    ...
    const result = await accountsService.createAccountReceivable(
      accountData,
      token
    );
...
export async function uploadAccounts(req, res) {
  ...
  try {
    const results = await accountsService.processExcelAccountsReceivable(
      req.file.buffer,
      token
    );
...
export async function uploadUpdateAccounts(req, res) {
  ...
  try {
    const results = await accountsService.updateAccountsReceivable(
      req.file.buffer,
      token
    );
    res.status(200).json(results);
...
export async function baixarContas(req, res) {
  console.log("Headers:", req.headers);
  console.log("File:", req.file); // Verifique se aparece algo aqui
  const token = req.headers.authorization?.split(" ")[1];
  ...
  try {
    const results = await accountsService.baixarAccountsReceiveable(
      req.file.buffer,
      token
    );
    res.status(200).json(results);
...
export async function downloadTemplate(req, res) {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=template_contas_receber.xlsx"
    );
...
export async function downloadTemplateUpdate(req, res) {
  ...
  try {
    const template = await accountsService.getTemplateUpdateAccountsReceivable(
      token
    );
    res.setHeader(
      "Content-Type",
