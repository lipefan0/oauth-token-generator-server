...
export async function processExcelAccountsReceivable(buffer, token) {
  ...
  if (worksheet.rowCount <= 1) {
    return {
      ...
      error:
        "A planilha não contém contas para importar. Por favor, utilize o template fornecido.",
    };
  }
  ...
  async function tryCreateAccount(account, rowIndex, attempt = 1) {
    try {
      ...
      if (attempt < 4) {
        ...
        console.log(
          `  → Aguardando ${waitTime / 1000}s antes da próxima tentativa...`
        );
  ...
  for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
    ...
    try {
      const account = {
        ...
        competencia:
          competencia instanceof Date
            ? competencia.toISOString().split("T")[0]
            : competencia,
        dataEmissao:
          dataEmissao instanceof Date
            ? dataEmissao.toISOString().split("T")[0]
            : dataEmissao,
        valor: Number(valor),
        contato: {
          id: Number(contatoId),
      ...
      console.log(
        `✗ Linha ${rowNumber}: Falha após todas as tentativas:`,
        errorMessage
      );
...
export async function getExcelTemplate(token) {
  ...
  try {
    ...
    for (
      let i = worksheetPrincipal.rowCount - 4;
    ...
    worksheetPrincipal.addRow([]);
    worksheetPrincipal.addRow([
      "Não foi possível carregar os dados de referência. Por favor, gere o template novamente.",
    ]);
  }
...
export async function getTemplateUpdateAccountsReceivable(token) {
  ...
  while (true) {
    const response = await fetch(
      `https://www.bling.com.br/Api/v3/contas/receber?pagina=${page}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
...
export async function updateAccountsReceivable(buffer, token) {
  ...
  if (worksheet.rowCount <= 1) {
    return {
      message: "Planilha vazia",
      error:
        "A planilha não contém contas para atualizar. Por favor, utilize o template fornecido.",
    };
  }
  ...
  async function tryUpdateAccount(account, rowIndex, attempt = 1) {
    try {
      ...
      const response = await fetch(
        `https://www.bling.com.br/Api/v3/contas/receber/${account.idContaReceber}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      console.log(
        `! Linha ${rowIndex}: Tentativa ${attempt} falhou - ${error.message}`
      );
...
export async function baixarAccountsReceiveable(buffer, token) {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load(buffer);
  ...
  async function tryDownloadAccount(account, rowIndex, attempt = 1) {
    try {
      await delay(1500); // 1.5 segundos entre requisições

      const response = await fetch(
        `https://www.bling.com.br/Api/v3/contas/receber/${account.idContaReceber}/baixar`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      console.log(`✓ Linha ${rowIndex}: Registro baixado com sucesso`);
      return { success: true, data: responseData };
    } catch (error) {
      console.log(
        `! Linha ${rowIndex}: Tentativa ${attempt} falhou - ${error.message}`
      ...
      if (attempt < 4) {
        ...
        await delay(waitTime);
        return tryDownloadAccount(account, rowIndex, attempt + 1);
      }
      throw error;
    }
  }
  ...
  for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
    ...
    const idContaReceber = row.getCell("A").value;
    const dataPagamento = row.getCell("B").value;
    const usarDataVencimento = row.getCell("C").value;
    const portadorId = row.getCell("D").value;
    const categoriaId = row.getCell("E").value;
    const historico = row.getCell("F").value;
    ...
    try {
      const payload = {
        data:
          dataPagamento instanceof Date
            ...
            : dataPagamento,
        usarDataVencimento: usarDataVencimento === "SIM",
        portador: portadorId ? { id: Number(portadorId) } : undefined,
        categoria: categoriaId ? { id: Number(categoriaId) } : undefined,
        historico: historico || undefined,
      };
      ...
      const camposFaltantes = [];
      if (!payload.data) camposFaltantes.push("Data de Pagamento");
      ...
      const result = await tryDownloadAccount(account, rowNumber);
      results.success++;
      results.processedItems.push({
        index: rowNumber - 1,
        status: "success",
